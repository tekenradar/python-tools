...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--folder_with_all_templates", help="folder containing the email template")
    ...
    root = args.folder_with_all_templates

    client = ManagementAPIClient(management_api_url, user_credentials, use_external_idp=use_external_idp)

    for f in os.listdir(root):

        email_template_folder = os.path.join(root, f)

        conf_path = os.path.join(email_template_folder, 'email-props.yaml')
        if not os.path.exists(conf_path):
            continue

        print('\nProcessing {}'.format(f))
        email_config = read_yaml(conf_path)
